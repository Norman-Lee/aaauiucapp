	...
	describe "POST #create" do
		context "when is successfully created" do
			...
			it "renders the json representation for member record just created" do
				...
				expect(member_response[:e_mail]). to eql @member_attributes[:e_mail]
			end
		...
		context "when not successfully created" do
			before(:each) do
				...
				post :create, { member: @invalid_member_attributes }
			end
			...
			it "tells why the member record could not be created in json" do
				member_response = json_response
				expect(member_response[:errors][:e_mail]).to include 'does not appear to be a valid e-mail address'
			end
